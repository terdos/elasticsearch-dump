...
class csv extends base {
  ...
  set (data, limit, offset, callback) {
    ...
    if (!this.stream) {
      ...
      if (!this.parent.options.overwrite && fs.existsSync(this._fileObj.path)) {
        ...
        const fileStream = fs.createWriteStream(this._fileObj.path)
        _csvStream.pipe(_throughStream).pipe(fileStream)
    ...
    if (data.length === 0) {
      ...
      // Honour stream backpressure. Acknowledging the write before the CSV
      // stream has drained lets the processor keep reading from the source,
      // buffering the whole dataset in memory on a slow sink (OOM on large
      // dumps). Wait for `drain` when the buffer is full instead.
  ...
  finalizeStream (callback, lineCounter) {
    if (this.shouldSplit) {
      ...
      const streamToWait = this.fileStream || this._throughStream || this.stream
